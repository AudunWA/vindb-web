...
router.get('/', function (req, res, next) {
  var squelQuery = squel.select({ autoQuoteAliasNames: false })
    ...
    .join("field", null, "field.field_id = pc.field_id")
    .join("change_log", "cl", "cl.change_id = pc.change_id")
    .join("product", "p", "p.varenummer = pc.product_id")
    .order("time", false);
  ...
  if (req.query.start_date) {
    if (req.query.end_date) {
      squelQuery
        .where("DATE(cl.time) BETWEEN ? AND ?", req.query.start_date, req.query.end_date)
        .limit(1000); // Workaround for really big page
    }
  }
  else {
